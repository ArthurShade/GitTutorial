...
GameBase::GameBase(void){
	...
	fs_white = new AWFont("Framework/vera_white.bmp", "Framework/vera.dat");
	fs_green = new AWFont("Framework/vera_green.bmp", "Framework/vera.dat");
	fs_red = new AWFont("Framework/vera_red.bmp", "Framework/vera.dat");
}
...
void GameBase::setBackground(char *imageFileName){
	...
	if(temp != NULL){
		...
		cout << "Unable to locate image file </3" << imageFileName << endl;
	}
}
