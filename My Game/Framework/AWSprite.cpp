// Copyright Andrew Williams, Peter Lager and the University of Bolton
...
void AWSprite::makeDefaultImage(int w, int h, Uint32 red, Uint32 green, Uint32 blue){
	...
	SDL_Surface *temp;
	temp = SDL_CreateRGBSurface(screen->flags, w, h,
...
SDL_Surface** AWSprite::create_frames_with_alpha(Uint32 cols, Uint32 rows) {
	...
	amask = 0xff000000;
#endif
	// Clear SDL_SRCALPHA flag to get raw copy
	wholeSurface->flags &= (0xffffffff - SDL_SRCALPHA);

	//SDL_Surface *screen = SDL_GetVideoSurface();
	SDL_Surface *temp;
	for(Uint32 c=0; c < cols; c++) {
		for(Uint32 r=0; r < rows; r++) {
			temp = SDL_CreateRGBSurface(SDL_SWSURFACE | SDL_SRCALPHA, width, height,
										 32,
										 rmask, gmask, bmask, amask);
...
void AWSprite::set_surface_alpha(Uint32 value) {
	if(value < 0) 
		value = 0;
	else
		value %= 256;
...
void AWSprite::restore_images() {
	...
	bool usingColorKey = ((wholeSurface->flags & SDL_SRCCOLORKEY) == SDL_SRCCOLORKEY);
	// Get the flags for alpha control and the per-surface colorkey 
	Uint32 alphaFlags = (wholeSurface->flags & (SDL_SRCCOLORKEY | SDL_RLEACCEL));
	Uint32 alphaColorKey = wholeSurface->format->colorkey;
